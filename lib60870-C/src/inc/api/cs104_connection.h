...
/**
 * \brief Select the A-profile key establishment algorithm
 *
 * Choose between 1) Classical ECDH and 2) Post-Quantum KEM (Kyber)
 * If Kyber is not available at build-time, the library will fall back to ECDH.
 *
 * \param self CS104_Connection instance
 * \param algorithm 1 = ECDH, 2 = Kyber
 */
void
CS104_Connection_setSecurityAlgorithm(CS104_Connection self, int algorithm);
