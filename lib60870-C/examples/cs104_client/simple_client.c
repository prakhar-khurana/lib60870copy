...
static void rawMessageHandler2(void* parameter, uint8_t* msg, int msgSize, bool sent) {
    const char* type = "Unknown";
    if (msgSize > 0) {
        switch(msg[0]) {
            case S_RP_NA_1: type = "Security/Keying (136)"; break;
            case S_SE_NA_1: type = "Secure Data (138)"; break;
        }
    }
    printf("%s %s: %d bytes\n", sent ? "Sent" : "Received", type, msgSize);
}
...
main(int argc, char** argv)
{
    ...
    printf("Choose the communication encryption protocol:\n");
    printf("1) Classical ECDH\n");
    printf("2) Post Quantum KEM\n");
    printf("> ");
    int choice = 1;
    char line[16];
    if (fgets(line, sizeof(line), stdin)) {
        int v = atoi(line);
        if (v == 2) choice = 2; else choice = 1;
    }
    ...
    CS104_Connection_setRawMessageHandler(con, rawMessageHandler2, NULL);

    /* set desired security algorithm (A-Profile). It will fall back to ECDH if unavailable. */
    CS104_Connection_setSecurityAlgorithm(con, choice);
    /* enable A-Profile security using defaults (NULL configs); required to activate handshake */
